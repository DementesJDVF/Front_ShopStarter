        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-72 bg-white/95 dark:bg-slate-900/95 backdrop-blur-3xl shadow-[0_0_50px_rgba(10,1,74,0.3)] transform transition-transform duration-500 ease-in-out md:hidden border-l border-white/20 dark:border-slate-800">
            <div className="p-6 flex items-center justify-between border-b border-indigo-50 dark:border-slate-800">
              ...
              <button
                onClick={() => setIsMobileMenuOpen(false)}
                className="p-2 text-indigo-950 dark:text-white hover:bg-indigo-50 dark:hover:bg-slate-800 rounded-xl"
              >
            ...
            <nav className="p-6 space-y-3">
              {navLinks.map((link) => (
                <a
                  ...
                  href={link.href}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="block px-5 py-4 text-indigo-950 dark:text-white hover:bg-indigo-50 dark:hover:bg-slate-800 hover:text-indigo-600 rounded-2xl transition-all font-black uppercase tracking-widest text-xs"
                >
                  {link.name}
                </a>
              ))}
              ...
              <div className="pt-6 border-t border-indigo-50 space-y-4">
                <Link
                  to="/auth/login"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="block w-full text-center px-5 py-2 text-indigo-950 dark:text-white font-black uppercase tracking-widest text-xs hover:text-indigo-600"
                >
                  {t('nav.mobile_login')}
                </Link>
                <Link
