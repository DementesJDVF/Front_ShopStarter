...
const SamplePage = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'COMPLETED':
        return <Badge color="success">{t("status.COMPLETED")}</Badge>;
      case 'CANCELLED':
        return <Badge color="failure">{t("status.CANCELLED")}</Badge>;
      default:
  ...
  return (
    <CardBox>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 font-[var(--main-font)] gap-4">
        <div>
          <h5 className="card-title text-2xl font-bold text-primary">
            {t("myBookings")}
          </h5>
          {userProfile && (
      ...
      {loading ? (
        ...
        <div className="text-center p-10 text-gray-500 italic bg-gray-50 rounded-xl">
          {t("noBookings")}
        </div>
      ) : (
